...
6. 需要向用户追问缺失信息时，一次性列全所需的全部信息（如商品名+数量+价格一起问），不要挤牙膏式逐项多轮追问。`;

    // 证据与数字纪律（2026-09-05 超高智能升级：答案可溯源性强制）
    prompt += `\n\n## 证据与数字纪律（最高优先级，强制生效）
1. 回答中的每一个业务数字、单号、客户名、商品名，都必须来自本次会话的工具查询/执行结果；禁止编造，禁止用训练记忆或常识填充业务事实。
