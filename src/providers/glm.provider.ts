...
export class GlmProvider implements IModelProvider {
  ...
  async *chat(
    ...
    this.logger.debug(
      `流式调用 GLM：model=${cfg.model}, messages=${messages.length}条`,
    );
  ...
  async chatSync(
    ...
    this.logger.debug(
      `非流式调用 GLM：model=${cfg.model}, messages=${messages.length}条`,
    );
    ...
    return {
      ...
      // GLM 返回的 finish_reason 为 string，断言为字面量联合（运行时由解析层保证值域）
  ...
  embedding(_text: string): Promise<number[]> {
    throw new ProviderError(
      'GLM 暂不支持 embedding，请使用 Ollama Provider',
      501,
      this.name,
    );
  }
  ...
  async testConnection(): Promise<ConnectionTestResult> {
    if (!this.config.apiKey) {
      return {
        success: false,
        message: '未配置 GLM_API_KEY，请在 .env 中设置后重启服务',
        latencyMs: 0,
      };
    }
    ...
    try {
      ...
      return {
        success: true,
        message: `GLM 连接正常（model=${this.config.model}, reply="${result.content.slice(0, 50)}"）`,
        latencyMs,
      };
    } catch (err) {
      const latencyMs = Date.now() - start;
  ...
  private assertConfigured(): void {
    if (!this.configured) {
      throw new ProviderError(
        'GLMProvider 尚未配置，请通过 ProviderFactory.create() 初始化',
        500,
        this.name,
      );
    }
