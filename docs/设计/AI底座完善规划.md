...
- **批次 6（安全加固 + E5 自治闭环）✅ 已完成（2026-09-05）**：全面审查后鉴权链收紧（商家/平台双 JWT 验签、验签失败 401、身份只认 JWT、AdminGuard 挂全部管理端点、MCP Token 哈希化+列表脱敏、CORS 白名单、trust proxy）；E5 从模拟框架补完为真实评测（StructuredExtractor 逐字段比对 + 95% 回归达标线 + 结果落库迁移 007 + 总台策略门控 evolution_auto_activate 自动激活/拦截 + `POST /api/admin/ai-db/versions/:id/auto-close`）；依赖通告清零（qs/@xmldom overrides）；桌面客户端登录三件套（登录/注册/演示登录）+ 401 自愈。遗留 P0：生产 AI 底座与后端 JWT_SECRET 不一致待服务器同步（客户端与本地代码已就绪，见审查记录）。
